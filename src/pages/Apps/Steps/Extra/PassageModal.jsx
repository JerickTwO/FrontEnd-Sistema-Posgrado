...
const PassageModal = ({ isOpen, onClose, onSave, passage }) => {

    const initialValues = React.useMemo(
        () => ({
            meetRequirements: passage?.meetRequirements ? 'yes' : 'no',
            observation: passage?.observation || '',
        }),
        [passage]
    );
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        ...
        try {
            await onSave(passage?.id, normalizedValues);
            Swal.fire('Éxito', 'El pasaje fue guardado correctamente.', 'success');
        } catch (error) {
            Swal.fire('Error', 'Ocurrió un error al guardar el pasaje.', 'error');
        } finally {
            setSubmitting(false);
        }
    };
